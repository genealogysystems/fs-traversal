...
module.exports = function(sdk) {
  ...
  return {
    ...
    family: function(func) {this._registerCallback('family', func); return this;},
    ...
    _processPerson: function(person) {
      ...
      // For this person and all related persons, piece together all the
      // different families they belong to and call the family callback if
      // all persons in that family have been visited.
      var personIds = [id]
        .concat(person.getMotherIds())
        .concat(person.getFatherIds())
        .concat(person.getSpouseIds())
        .concat(person.getChildIds());

      // This object will be keyed by {wifeId:husbandId}.
      var families = {};  
        
      each(personIds, function(personId){
        
        // Only process people that have been visited
        if(self._visited[personId]) {
          
          // Piece together families this person belongs to by examining
          // all ChildAndParents relationships it plays a role in
          var childAndParentsList = self._visited[personId].getChildRelationships()
            .concat(self._visited[personId].getParentRelationships());
      ...
      // For each resulting family, check whether all persons have been visited
      each(families, function(family){
        ...
        each(_unique(family.childrenIds), function(childId){
          if(!self._visited[childId]){
            allVisited = false;
          } else {
            children.push(self._visited[childId].getPrimaryPerson());
