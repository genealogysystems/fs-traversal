...
module.exports = function(sdk) {
  ...
  return {
    ...
    _processPerson: function(person) {
      ...
      var filtered = [rels];
      for(var x in self._callbacks.filter) {
        // Pass in a copy of rels
        filtered.push(self._callbacks.filter[x].call(self, JSON.parse(JSON.stringify(rels))));
      }
      ...
      each(childParents, function(childParent){
        if(self._visited[childParent.$getChildId()] 
            && (!childParent.$getMotherId() || self._visited[childParent.$getMotherId()])
            && (!childParent.$getFatherId() || self._visited[childParent.$getFatherId()]) ) {
          // Call the child callback
          (function(cid, mid, fid) {

            for(var y in self._callbacks.child) {
              setTimeout(function() {
                self._callbacks.marriage[y].call(self, self._visited[cid], self._visited[mid], self._visited[fid]);
              }, 0);
            }

          })(childParent.$getChildId(), childParent.$getMotherId(), childParent.$getFatherId());
          // Call the parent callback with mother
          (function(cid, mid) {

            for(var y in self._callbacks.child) {
              setTimeout(function() {
                self._callbacks.marriage[y].call(self, self._visited[mid], self._visited[cid]);
              }, 0);
            }

          })(childParent.$getChildId(), childParent.$getMotherId());
          // Call the parent callback with father
          (function(cid, fid) {

            for(var y in self._callbacks.child) {
              setTimeout(function() {
                self._callbacks.marriage[y].call(self, self._visited[fid], self._visited[cid]);
              }, 0);
            }

          })(childParent.$getChildId(), childParent.$getFatherId());
        }
      });
    },
