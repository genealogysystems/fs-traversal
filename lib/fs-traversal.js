  ...
  return {
    ...
    _relationshipTo: function(path) {
      ...
      var relation = '',
          ...
          // Represents the gender of the last person in the
          // portion of the relationships string that we are
          // currently working with. We need to know the gender
          // to differentiate between aunt and uncle.
          gender,
          
          // Represents the starting location of the portion
          // of the string that we are currently evaluating.
          // This is primarily used to calculate lastPersonOffset
          offset = 1,
          
          // Pointer to the last person in the portion of the
          // relationship that we are currently examining.
          // This is primarly used to calculate the gender.
          lastPersonOffset; 
      ...
      while(!done) {
        ...
        match = true;
        gender = undefined;
        lastPersonOffset = offset + switchStr.length - 1;
        ...
        // We know we have a grandparent if the path length is greater than 2
        // and all relationships are parent relationships
        if(switchStr.length >= 2 && switchStr.replace(/↑/g,'').length === 0) {
          
          var pLength = switchStr.length;
